...
def update_labels_to_ask(edit_mode: bool = False):
    """
    Determine which labels to ask next.
    - Non-edit mode: only ask labels whose value is 'none'.
    - Edit mode: ask all labels.
    """
    labels_to_ask = []
    for key in OPTIONS:
        if edit_mode or st.session_state.completed_tags.get(key, 'none') == 'none':
...
def reset_chat():
    ...
    st.session_state.labels_to_ask_initialized = None
...
# 1.5. Processing step (detect tags and generate questions)
    ...
    if st.session_state.processing:
        with st.spinner("Analyzing your input and preparing questions..."):
            ...
            # Determine labels to ask (only 'none' ones)
            update_labels_to_ask(edit_mode=False)

            # Generate questions
            if st.session_state.labels_to_ask:
                st.session_state.generated_questions = generate_all_questions(
                    st.session_state.labels_to_ask, st.session_state.completed_tags
                )
...
# 2. Ask for missing labels
    ...
    # Check if edit_mode is enabled
    edit_mode = st.session_state.get("edit_mode", False)

    # Initial labels_to_ask
    if "labels_to_ask_initialized" not in st.session_state or st.session_state.labels_to_ask_initialized != edit_mode:
        update_labels_to_ask(edit_mode=edit_mode)
        st.session_state.labels_to_ask_initialized = edit_mode
        st.session_state.current_label_index = 0

    # If no labels need to be asked, skip to step 2
    ...
    # Display generated question if exists
    if current_label in st.session_state.generated_questions:
        st.markdown(f"**{st.session_state.generated_questions[current_label]}**")

    # Hints for 'phases'
    if current_label == "phases":
        with st.expander("ℹ️ Hints for choosing phase"):
            st.markdown("""
            - **Feedback**: Review current/past events and gather factual info.
            - **Reflection**: Analyze experiences and identify areas for improvement.
            - **Planning**: Develop concrete action plans for future practices.
            """)

    # Hints for 'dimensions'
    if current_label == "dimensions":
        with st.expander("ℹ️ Hints for choosing dimension"):
             st.markdown("""
            - **Social**: About team, coordination and communication.
            - **Process**: About project, procedure and activity.
            - **Goal**: About product, solution and task.
            """)
...
# 3. Show collected information and generate questions
    ...
    with context_cols[0]:
        st.info(f"**Domain**: {st.session_state.completed_tags.get('domains','none')}")
        st.info(f"**Subdomain**: {st.session_state.completed_tags.get('subdomains','none')}")
        st.info(f"**Phase**: {st.session_state.completed_tags.get('phases','none')}")
    with context_cols[1]:
        st.info(f"**Dimension**: {st.session_state.completed_tags.get('dimensions','none')}")
        st.info(f"**Level**: {st.session_state.completed_tags.get('level','none')}")
        st.info(f"**Objective**: {st.session_state.completed_tags.get('objectives','none')}")
